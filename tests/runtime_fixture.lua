...
function mock.surface(name,index)
  ...
  s.find_entities_filtered=function(filter)
    ...
    for _,e in ipairs(s.entities) do
      ...
      if e.valid and in_range and matches(e.name,filter.name) and matches(e.type,filter.type) and matches(e.force.name,filter.force) and (not filter.to_be_deconstructed or (e.to_be_deconstructed and e.to_be_deconstructed())) and (not filter.to_be_upgraded or (e.to_be_upgraded and e.to_be_upgraded())) then
...
function mock.run(ticks)
  for _=1,ticks do
    ...
    for _,interval in ipairs({1,3,6,15,60,120}) do if game.tick%interval==0 and mock.nth[interval] then mock.nth[interval]({tick=game.tick}) end end
  end
end
mock.gui_reserved = {}
