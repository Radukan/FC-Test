...
def generate():
    ...
    def put(section,key,value): sections.setdefault(section,OrderedDict())[key]=str(value).replace('\n','\\n')
    for x in k['items']:
    ...
    put('entity-description','sn-native','A native organism resisting ecological change. Mobilized only at an existing nest, with a warning; not a random spawn inside your factory.')
    for x in k['recipes']:
        ...
        if x.get('effects'):
            ...
            put('recipe-description','sn-'+x['name'],f"Per completed {x['seconds']}-second cycle at crafting speed 1:\n"+' · '.join(parts)+'.\nFitness gains use planetary multipliers and ecological support ceilings. Unpowered, ingredient-starved or output-blocked machines earn nothing.')
