...
def run(binary:Path):
    ...
    for source in (ROOT/'tests/engine').glob('*.lua'):
        shutil.copyfile(source,test/source.name)
    ...
    outputs=[]
    for label,args in [('create',['--create',str(world)]),('benchmark',['--benchmark',str(world),'--benchmark-ticks','6000','--benchmark-runs','1'])]:
        command=base+args
